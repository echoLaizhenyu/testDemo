...
//					body = EntityUtils.toString(entity, charset.toString());
	...
	public  Article getNewArticle(int readNum) throws Exception{
...
//		System.out.println(doGet);
        ...
        if(readNum<num){//表示有未读章节
        	String doGet2 = doGet.substring(doGet.indexOf("第"+(readNum+1)+"章")-40,doGet.indexOf("第"+(readNum+1)+"章")+40);
        	String regex3 = "(<dd>)(.*?)(第"+(readNum+1)+"章)(.*?)(</a></dd>)";
            String newUrl = getMatcher(regex3, doGet2,2);
            String newName = getMatcher(regex3, doGet2,4);
            
            String regex_url = "(\")(.*?)(\")";
            
            artticle.setUrl("http://www.biquyun.com/"+getMatcher(regex_url,newUrl,2));
            artticle.setName(newName);
            artticle.setArticleNum(readNum+1);
            System.out.println("======="+newUrl);
        }else{
        	artticle.setArticleNum(num);
        	artticle.setName(name);
        }
		return artticle;
	}
	public  String getMatcher(String regex, String source,int num) {  
        String result = "";  
	...
	class Article{
		...
		public String toString() {
			return "Article [url=" + url + ", articleNum=" + articleNum + ", name=" + name + "]";
		}
		int articleNum;
		String name;
	...
	public void testfff() throws Exception{
		Article newArticle = getNewArticle(635);
		System.out.println(newArticle.toString());
		//这里给用户推送一下
		
		//把内容拔取下来
		String doGet = doGet(newArticle.getUrl()).substring(3000);
		//<div id="content">  </div>
		String regex = "(<divid=\"content\">)(.*?)(</div>)";
	    String matcher = getMatcher(regex, getStringNoBlank(doGet),2);
//		System.out.println(getStringNoBlank(doGet));
		System.out.println("---该看的----"+matcher);
	}
